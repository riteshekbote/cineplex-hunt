...
## RANKED HYPOTHESES 2026-09-24 12:34:20 UTC
- [97] graphql-api.app.cineplex.de: Systemic Unauthenticated IDOR Across 4 Single-Entity Resolvers via decodePublicId-Before-Gate (from art/lead_nemotron3.txt)
- NEXT(hypotheses-nemotron3.txt): HUMAN: submit 4-finding bundle to bugs.olivermaicher.eu with evidence packages — (1) dangling CNAME web-dev.cineplex.de (DoH logs + claimability attestation tem
- LEARN: ACCEPTED ct_diff_sweep @ *.cineplex.de: full passive CT sweep (certspotter+hackertarget+systems-zone) this cycle → zero non-inventory hosts; only wildcard certs
- LEARN: REJECTED www.support.systems.cineplex.de @ dangling-CNAME-lookalike: cert SAN present but DoH Status 3 NXDOMAIN with no CNAME record — host-only dead entry, not
...
## RANKED HYPOTHESES 2026-09-24 17:32:57 UTC
- [97] graphql-api.app.{,staging.}cineplex.de: Systemic unauth'd IDOR via decodePublicId-before-gate (report-ready, unchanged) (from art/lead_bigpickle.txt)
- [95] web-dev.cineplex.de: Dangling CNAME Takeover on web-dev.cineplex.de → Azure Container Apps (from art/lead_nemotron3.txt)
- NEXT(hypotheses-bigpickle.txt): HUMAN: submit 3-finding bundle to bugs.olivermaicher.eu — (1) web-dev.cineplex.de dangling CNAME→azurecontainerapps.io (Medium, PASSIVE, fresh DoH JSON CNAME St
- NEXT(hypotheses-nemotron3.txt): HUMAN: submit 4-finding bundle to bugs.olivermaicher.eu with evidence packages — (1) dangling CNAME web-dev.cineplex.de (DoH logs + claimability attestation tem
- LEARN: ACCEPTED e2e_convergence @ cineplex: inventory 132-host baseline + CT diff + CNAME sweep exhausted; correct behavior is no-op on empty delta, not fabricated fin
- LEARN: REJECTED all prior OOS classes (username_enumeration, ssl_tls_best_practices, csrf_logout, descriptive_errors, known_vuln_library, OAuth/JWKS-passive, api.cinep
- LEARN: ACCEPTED ct_diff_sweep @ *.cineplex.de: full passive CT sweep (certspotter+hackertarget+systems-zone) this cycle → zero non-inventory hosts; only wildcard certs
- LEARN: REJECTED www.support.systems.cineplex.de @ dangling-CNAME-lookalike: cert SAN present but DoH Status 3 NXDOMAIN with no CNAME record — host-only dead entry, not
- LEARN: REJECTED ct-renewal_signal @ booking-dev/bms-dev (09-21), profil.cineplex.de (09-22): cert rotation within expected lifecycle, no new SANs, no surface change
...
## RANKED HYPOTHESES 2026-09-24 20:45:49 UTC
- [98] graphql-api.app.cineplex.de: GraphQL Introspection + Full Mutation Surface Exposure on Production (from art/lead_nemotron3.txt)
- [97] graphql-api.app.{,staging.}cineplex.de: Systemic unauth'd IDOR via decodePublicId-before-gate (report-ready, unchanged) (from art/lead_bigpickle.txt)
- NEXT(hypotheses-bigpickle.txt): HUMAN: submit 3-finding bundle to bugs.olivermaicher.eu — (1) web-dev.cineplex.de dangling CNAME→azurecontainerapps.io (Medium, PASSIVE, fresh DoH JSON CNAME St
- NEXT(hypotheses-nemotron3.txt): HUMAN: submit 4-finding bundle to bugs.olivermaicher.eu with evidence packages — (1) dangling CNAME web-dev.cineplex.de (DoH logs + claimability attestation tem
...
## RANKED HYPOTHESES 2026-09-24 23:45:01 UTC
...
## RANKED HYPOTHESES 2026-09-25 03:10:56 UTC
- [98] graphql-api.app.cineplex.de: GraphQL Full Introspection + Mutation Surface Exposure on Production (from art/lead_nemotron3.txt)
- [97] graphql-api.app.{,staging.}cineplex.de: Systemic unauth'd IDOR via decodePublicId-before-gate (report-ready, unchanged) (from art/lead_bigpickle.txt)
- NEXT(hypotheses-bigpickle.txt): HUMAN: submit 3-finding bundle to bugs.olivermaicher.eu — (1) web-dev.cineplex.de dangling CNAME→azurecontainerapps.io (Medium, PASSIVE, fresh DoH JSON CNAME St
- NEXT(hypotheses-nemotron3.txt): HUMAN: submit 4-finding bundle to bugs.olivermaicher.eu with evidence packages — (1) dangling CNAME web-dev.cineplex.de (DoH logs + claimability attestation tem
...
## RANKED HYPOTHESES 2026-09-25 08:57:17 UTC
- [98] graphql-api.app.cineplex.de: GraphQL Full Introspection + Mutation Surface Exposure on Production (from art/lead_nemotron3.txt)
...
- NEXT(hypotheses-bigpickle.txt): HUMAN: submit the existing three-finding evidence bundle to bugs.olivermaicher.eu with redacted evidence; mark the resolver finding HUMAN_ONLY for consented cro
- NEXT(hypotheses-nemotron3.txt): HUMAN: submit 4-finding bundle to bugs.olivermaicher.eu with evidence packages — (1) dangling CNAME web-dev.cineplex.de (DoH logs + claimability attestation tem
...
- LEARN: REJECTED relay_metrics, relay_broker_saturation @ data-9fc27eb430.cineplex.de: IOMB broker statistics are descriptive and expose no exploitable authless manipul
...
## RANKED HYPOTHESES 2026-09-25 18:52:39 UTC
- [99] graphql-api.app.{,staging.}cineplex.de: Unauthenticated full introspection exposes the complete 223-operation surface on production and staging (from art/lead_bigpickle.txt)
- [98] graphql-api.app.cineplex.de: GraphQL Full Introspection + Mutation Surface Exposure on Production (from art/lead_nemotron3.txt)
- NEXT(hypotheses-bigpickle.txt): HUMAN: submit the three-finding redacted evidence bundle to bugs.olivermaicher.eu — (1) introspection with this cycle's metadata-only 83-query/140-mutation oper
- NEXT(hypotheses-nemotron3.txt): HUMAN: submit 4-finding bundle to bugs.olivermaicher.eu with evidence packages — (1) dangling CNAME web-dev.cineplex.de (DoH logs + claimability attestation tem
- LEARN: ACCEPTED graphql_introspection @ graphql-api.app.{,staging.}cineplex.de: upgraded to same-cycle GET verification — the first cycle where the evidence is not man
...
- LEARN: ACCEPTED dangling_cname_takeover @ web-dev.cineplex.de: 13th+ consecutive NXDOMAIN, and the pipeline's DoH probe now succeeds (200) after 10 cycles of 415s — th
- LEARN: REJECTED staging_testing_oracle @ graphql-api.app.staging.cineplex.de: the field exists in prod's queryType too and no code was ever extracted; a method-mismatc
...
## RANKED HYPOTHESES 2026-09-25 21:58:17 UTC
...
## RANKED HYPOTHESES 2026-09-26 05:31:14 UTC
- [100] github.com/Cineplex-Deutschland: No public repos available for audit (from art/lead_bigpickle.txt)
- [99] graphql-api.app.cineplex.de: GraphQL Full Introspection + Mutation Surface Exposure on Production (from art/lead_nemotron3.txt)
- NEXT(hypotheses-bigpickle.txt): HUMAN: file the redacted bundle to bugs.olivermaicher.eu as **four** findings — (1) introspection with this cycle's `__type` argument-type bodies and the single
- NEXT(hypotheses-nemotron3.txt): HUMAN: submit 3-finding redacted evidence bundle to bugs.olivermaicher.eu — (1) GraphQL introspection graphql-api.app.{,staging.}cineplex.de (schema dump + muta
- LEARN: ACCEPTED MISCONFIG @ graphql-api.app.{,staging.}cineplex.de: introspection upgraded again this cycle from field names to **argument types** — prod Query 200/130
- LEARN: REJECTED ssrf_getOnlineTicketingBooking @ graphql-api.app.cineplex.de: my own loopback canary returned `FORBIDDEN "You must be the root user"` identically with 
- LEARN: REJECTED deeplink_entity_id_oracle @ graphql-api.app.cineplex.de: `externalUrl`/`appDeepLink` are unauthenticated but their path vocabulary exposes only `movie`
- LEARN: ACCEPTED BUSLOGIC @ graphql-api.app.cineplex.de: new attack surface documented at argument level — `startWebBooking(linkedUsersIds:[ID]!, freeTicketSpend:Int)` 
- LEARN: CORRECTION @ knowledge base: the 2026-09-12→09-25 entry "full mutation arg enumeration (35KB) confirms no injection vectors, all args scalar/named input objects
- LEARN: ACCEPTED e2e_convergence @ cineplex: automated `probe-results.md` unchanged since 00:24 UTC (986 lines, still zero POST), the 01:56 triage is an `UnknownError` 
- LEARN: REJECTED relay_metrics, relay_broker_saturation @ data-9fc27eb430.cineplex.de: IOMB broker counters are descriptive telemetry with no unauthenticated manipulati
...
- LEARN: REJECTED username_enumeration, ssl_tls_best_practices, csrf_logout, descriptive_errors, known_vuln_library @ all: explicit program exclusions, unchanged. No res
...
- LEARN: REJECTED username_enumeration, ssl_tls_best_practices, csrf_logout, descriptive_errors, known_vuln_library @ all: explicit program exclusions, unchanged.
- LEARN: REJECTED TLS-dead hosts @ app.staging.cineplex.de, graphql-api.app.couat.cineplex.de, login.cineplex.de, sso.cineplex.de: no reachable web surface.
- LEARN: ACCEPTED graphql_introspection @ graphql-api.app.{,staging.}cineplex.de: upgraded again from field names to ARGUMENT TYPES - prod Query 200/13070B/83 fields, pr
- LEARN: REJECTED ssrf_getOnlineTicketingBooking @ graphql-api.app.cineplex.de: my own loopback canary returned FORBIDDEN "You must be the root user" identically with ar
- LEARN: REJECTED deeplink_entity_id_oracle @ graphql-api.app.cineplex.de: externalUrl/appDeepLink are unauthenticated but their path vocabulary exposes only `movie` (us
- LEARN: ACCEPTED buslogic_booking_mutations @ graphql-api.app.cineplex.de: new attack surface documented at argument level - startWebBooking(linkedUsersIds:[ID]!, freeT
...
- LEARN: ACCEPTED e2e_convergence @ cineplex: automated probe-results.md unchanged since 00:24 UTC, the 01:56 triage is an UnknownError server fault, and reposcan is sti
- LEARN: REJECTED relay_metrics, relay_broker_saturation @ data-9fc27eb430.cineplex.de: IOMB broker counters are descriptive telemetry with no unauthenticated manipulati
- LEARN: REJECTED api_cineplex_get_bypass @ api.cineplex.de: strict 403 across every method and encoding across 20+ cycles. Hypothesis dead.
- LEARN: REJECTED username_enumeration, ssl_tls_best_practices, csrf_logout, descriptive_errors, known_vuln_library @ all: explicit program exclusions, unchanged. No res
- LEARN: REJECTED TLS-dead @ app.staging.cineplex.de, graphql-api.app.couat.cineplex.de, login.cineplex.de, sso.cineplex.de: no reachable web surface.
